// VIBEMATCH DJ SYSTEM (BINARY SEARCH TREE #4)
...
SongNode* insertTrack(SongNode *root, int bpm, char *title) {
    // BASE CASE
    if (root == NULL){
        SongNode* song = createNode(bpm, title);
        root = song;
        return root;
    }

    // CASE: LOCATING EMPTY SPOT
    if (bpm < root->bpm){
...
SongNode* findTrack(SongNode *root, int targetBPM) {
    ...
    // IF: SONG FOUND
    if (root->bpm == targetBPM)
        return root;
    
    // ELSE: MOVE DOWNWARDS
    if (targetBPM < root->bpm){
        return findTrack(root->left, targetBPM);
    }
    else if (targetBPM > root->bpm){
        return findTrack(root->right, targetBPM);
    }
}
...
SongNode* getEnergyBoost(SongNode *root, int currentBPM) {
    ...
    while (curr != NULL){
        if (curr->bpm <= currentBPM){
            curr = curr->right;
        }

        else if (curr->bpm > currentBPM){
            boost = curr;
            curr = curr->left;
        }
    }
