...
EventNode* archiveEvent(EventNode *root, int year, char *desc) {
    // IF: EMPTY = INSERT
    if (root == NULL){
        EventNode* event = createEvent(year, desc);
        root = event;
        return root;
    }

    // ELSE: MOVE ONE LEVEL LOWER
    if (year < root->year){
...
int countEventsInEra(EventNode *root, int startYear, int endYear) {
    ...
    if (root->year >= startYear && root->year <= endYear){
        count++;
...
EventNode* eraseEvent(EventNode *root, int year) {
    ...
    // CASE: NODE FOUND!
        ...
        EventNode* successor = findMinNode(root->right);
        strcpy(root->description, successor->description);
        root->year = successor->year;
        root->right = eraseEvent(root->right, successor->year);
        return root;
    }
}
